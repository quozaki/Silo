...
- **Empty states (3 tiers):**
  ...
  2. **Game selected but no env open / no browser (`!hasActiveBrowser && games.length>0 && selectedGameId`):** show `.ws-hint-card` variant `360 max-w pad 28 radius 16 bg --surface 1px --border L3`. Content: icon `44 radius 10 bg --surface-2 1px --border color --text-dim` (20-glyph), title `14/600 --text`, desc `12.5/400 --text-muted line 1.6`, kbd tip `11 mono --text-dim + kbd pills`. Title = `Environments for <GameName>` or fallback `Select an environment`. Desc changes if zero envs: "No environments yet — add one above." If envs exist: "Choose an environment from the sidebar to launch its isolated browser…"
